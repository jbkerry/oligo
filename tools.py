...
def check_off_target(genome, fa='', s_idx='', blat=False):
    ...
    Notes
    -----
    Two main output files are generated that are used by the
    `get_density()` function: oligo_seq.fa.out (the RepeatMasker output
    file) and either tiled_Aligned.out.sam (the STAR alignment file) OR
    blat_out.psl (the alignment file generated by BLAT). Anumber of
    other files are produced from these programs but are not required in
    this pipeline. They are not deleted in case the user is interested
    in their contents.
